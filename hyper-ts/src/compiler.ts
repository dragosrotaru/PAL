  ...
  compile(node: AST) {
    if (node instanceof ASTEdge) {
      ...
      try {
        ...
        throw new Error("Unknown Type", { cause: error });
      }
    }
    return node;
  }
...
export class UserInterface {
  ...
  selectInterface(obj: Obj) {
    ...
    const firstKnownInterface = type.data.knownInterfaces[0];
    if (!firstKnownInterface) throw new Error("No known interfaces");
    const result = this.interfaces.get(firstKnownInterface);
