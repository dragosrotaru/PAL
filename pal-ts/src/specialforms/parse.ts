/**
 * `(parse string)` special form — parses a raw string using the default PAL parser
 * and returns the resulting AST. Useful for eval-ing dynamically constructed code strings.
 * @author claude
 */
...
export const Is = (ast: Lang.AST): ast is Form =>
  STATIC.IsList(ast) && ast.length === 2 && ast[0] === Identifier && STATIC.IsString(ast[1]);
